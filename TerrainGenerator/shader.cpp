...
Shader::Shader(const std::string& vertexPath, const std::string& fragmentPath) {
    ...
    std::cout << "VERTEX Compiling" << std::endl;
    checkCompileErrors(vertex, "VERTEX");

    fragment = glCreateShader(GL_FRAGMENT_SHADER);
    glShaderSource(fragment, 1, &fShaderCode, NULL);
    glCompileShader(fragment);
    std::cout << "FRAGMENT Compiling" << std::endl;
    ...
    glLinkProgram(ID);
    std::cout << "PROGRAM Compiling" << std::endl;
...
void Shader::checkCompileErrors(unsigned int shader, const std::string& type)
{
    ...
    if (type != "PROGRAM") {
        ...
        if (!success) {
            ...
            glGetProgramiv(shader, GL_ATTACHED_SHADERS, &attachedShaders);
            if (attachedShaders > 0) {
                GLuint* shaders = new GLuint[attachedShaders];
                GLsizei count;
                glGetAttachedShaders(shader, attachedShaders, &count, shaders);

                for (int i = 0; i < count; ++i) {
                    GLint status;
                    glGetShaderiv(shaders[i], GL_COMPILE_STATUS, &status);
                    if (!status) {
                        std::cerr << "Shader " << shaders[i] << " failed to compile\n";
                    }
                }
                delete[] shaders;
            }
